...
3. Print a table: underlying | leveraged ETF | price | SMA10 | SMA30 | RSI14 |
   signal (BUY / SELL / HOLD) | reason. Always show this.
4. For each BUY/SELL signal: call get_equity_tradability, then review_equity_order,
   print the review, then place_equity_order with the correct order type for
   the session.
